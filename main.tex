...
\boldsymbol{a_i} = P_{ji}\boldsymbol{a^{'}_j}.
\end{equation}
...
\boldsymbol{A{^'}} = (P^T)^{-1} \boldsymbol{A}.
\end{equation}

Comparing \eqref{eqn:basis_transfromation2} we can see the the coordinates of a vector from $L_n$ transforms using a different transformation matrix when we move from an old basis $\boldsymbol{E}$ to a new one $\boldsymbol{E^'}$. This matrix can be obtained from the $P$ by taking the inverse of the transpose of $P$.
